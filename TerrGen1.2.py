...
from ScaledSum import ScaledSum
...
def spade_resblock(segmap, x_init, channels, use_bias=True, att_kernel=None, size=None):
    ...
    if not att_kernel==None:
        x = AttentionConv(channel_middle, att_kernel, bias=use_bias)(x)
    else:
        #x = conv(x, channels=channel_middle, kernel=3, stride=1, pad=1, use_bias=use_bias)
        x = Conv2DMod(filters = channel_middle, kernel_size=3, kernel_initializer='he_uniform')([x, spade_mod(segmap, x, channel_in, size)])
    ...
    #x = ScaledSum(size=size)(x)
    x = Conv2DMod(filters = channels, kernel_size=3, kernel_initializer='he_uniform')([x, spade_mod(segmap, x, channel_middle, size)])
...
def spade_mod(segmap, x_init, channels, size, use_bias=True):
    _, x_h, x_w, _ = tf.shape(x_init)
    _, segmap_h, segmap_w, _ = tf.shape(segmap)

    segmap_down = tf.image.resize(segmap, [x_h, x_w], method=tf.image.ResizeMethod.BILINEAR)
    segmap_down = conv(segmap_down, channels=channels, kernel=3, stride=1, use_bias=use_bias)
    segmap_down = tf.nn.relu(segmap_down)
    
    #x = tf.math.reduce_mean(x_init, axis=[1,2], keepdims=True)
    if size!=None and size>=8:
        x = ScaledSum(size)(x_init)
    else:
        x = x_init
    x = conv(x, channels, 1, 1) + 1
    segmap_gamma = tf.nn.softmax(x * segmap_down, axis=-1)
    #segmap_gamma = conv(segmap_gamma, channels=channels, kernel=3, stride=1, use_bias=use_bias)
    return segmap_gamma
...
def Generator():
    ...
    size = 8
    x = spade_resblock(segmap, x, channels=channel, use_bias=True, size=size)
    
    sf = 2
    x = tf.image.resize(x, [sf*z_height, sf*z_width], method=tf.image.ResizeMethod.BILINEAR)
    x = spade_resblock(segmap, x, channels=channel, use_bias=True, size=sf*size)
    x = spade_resblock(segmap, x, channels=channel, use_bias=True, size=sf*size)
    ...
    for i in range(4):
        ...
        x = tf.image.resize(x, [sf*z_height, sf*z_width], method=tf.image.ResizeMethod.BILINEAR)
        if sf<16:
            x = spade_resblock(segmap, x, channels=channel, use_bias=True, att_kernel=9, size=sf*size)
        else:
            x = spade_resblock(segmap, x, channels=channel, use_bias=True, size=None)
...
def Discriminator():
    ...
    for n in [size, size//4, size//16]:
        ...
        for i in range(d):
            ...
            #if i==0:
            #    x = AttentionConv(channel, 11, stride=stride, bias=True)(x)
            #else:
