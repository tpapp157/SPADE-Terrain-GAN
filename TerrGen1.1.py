...
from SPADE_convMod import Conv2DMod
from AttConv import AttentionConv
...
def spade_resblock(segmap, x_init, channels, use_bias=True):
    ...
    x = Conv2DMod(filters = channel_middle, kernel_size=3, kernel_initializer='he_uniform')([x, spade(segmap, x, channel_in)])

    x = tf.nn.leaky_relu(x, 0.2)
    x = Conv2DMod(filters = channels, kernel_size=3, kernel_initializer='he_uniform')([x, spade(segmap, x, channel_middle)])

# =============================================================================
#     if channel_in != channels :
#         x_init = Conv2DMod(filters=channels, kernel_size=1, kernel_initializer='he_uniform', dynamic=True)([x_init, spade(segmap, x_init, channel_in)])
# =============================================================================
...
def Generator():
    ...
    out = [tf.image.resize(conv(tf.nn.leaky_relu(x, 0.2), channels=OUTPUT_CHANNELS, kernel=1, stride=1, use_bias=True), [tf.shape(segmap)[1], tf.shape(segmap)[2]], method=tf.image.ResizeMethod.BILINEAR, antialias=True)]
    ...
    for i in range(4):
        ...
        #if np.mod(i,2)==1:
        out.append(tf.nn.leaky_relu(x, 0.2))
    ...
    for i in range(len(out)-1):
        out[i] = tf.image.resize(conv(out[i], channels=n, kernel=1, stride=1, use_bias=True), [tf.shape(segmap)[1], tf.shape(segmap)[2]], method=tf.image.ResizeMethod.BILINEAR, antialias=True)
    out = conv(tf.nn.leaky_relu(tf.add_n(out), 0.2), channels=OUTPUT_CHANNELS, kernel=1, stride=1, use_bias=True)

    return tf.keras.Model(inputs=segmap, outputs=out)
generator = Generator()
...
def vgg_layers(layer_names):
    ...
    outputs = [tf.reshape(vgg.get_layer(name).output, (BATCH_SIZE, -1)) for name in layer_names]
...
# 
# def Adaptive_Loss():
#     x = tf.keras.layers.Input(shape=[None], name='input_x')
#     d = np.array(tf.shape(x)[1]).astype(np.float32)
#     loss = AdaptiveLossFunction(d, tf.float32)(x)
#     return tf.keras.Model(inputs=x, outputs=loss)
# 
# AL = []
# for _ in range(len(style_layers)):
#     AL.append(Adaptive_Loss())
# =============================================================================
...
def vggloss(fake, real):
    ...
    for i in range(len(fake)):
        ...
        #loss += AL[i](fake[i] - real[i])
    return loss


#%%
checkpoint_dir = r'datasets\TERR\outH2\ckpt'
...
def generate_images(model, test_input, tar, htar, orig_inp, name):
    ...
    for i in range(out.shape.as_list()[0]):
        tf.io.write_file(os.path.join(r'datasets\TERR\outH2',str(name)+'-'+str(i)+'.png'), tf.image.encode_png(out[i,:,:,:]))
